...
class NeuralNetworkGUI(QWidget):
    ...
    def predict(self):
        if not self.model or len(self.selected_images) != 3:
            print("Сначала обучите модель и выберите ровно 3 изображения")
            return
        ...
        # Объединение изображений в один батч
        batch = np.array(processed_images)
        batch = np.expand_dims(batch, axis=-1)  # Добавляем канал для grayscale изображений

        # Предсказание для всех трех изображений одновременно
        predictions = self.model.predict(batch)

        for i, prediction in enumerate(predictions):
            print(f"Предсказание для изображения {self.selected_images[i]}: {prediction}")
        ...
        # Вывод топ-10 предсказаний
        top_10_indices = np.argsort(averaged_prediction)[-10:][::-1]
        ...
        for i, idx in enumerate(top_10_indices, 1):
            ...
            probability = averaged_prediction[idx]
            print(f"{i}. {class_name}: {probability:.4f}")

        # Определение итогового предсказанного класса
